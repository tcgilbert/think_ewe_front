...
const UserBookPost = (props) => {
    ...
    const postRating = () => {
        ...
        for (let i = 0; i < props.post.rating; i++) {
            stars.push(<img className="star-img-post" src={process.env.PUBLIC_URL + '/images/star.svg'} alt="star"/>)
        }
        return stars
    }
        ...
        <div className="book-post-container">
            <div className="cover-info-container-post">
                ...
                <div id="title-rating-author">
                    ...
                    <div className="book-post-rating">
                        {postRating()}
                    </div>
                </div>
            </div>
            <div className="post-social-content">
                <p>@{props.user.username}: {props.post.blurb}</p>
            </div>
        </div>
    );
};
